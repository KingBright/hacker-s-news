...
pub fn append_with_crossfade(all_samples: &mut Vec<f32>, new_samples: &[f32], sample_rate: usize, crossfade_duration: f64) {
...
pub async fn convert_to_mp3(wav_bytes: &[u8]) -> Result<Vec<u8>> {
    ...
    ).await.map_err(|_| anyhow::anyhow!("ffmpeg timeout 60s"))??;
