...
impl VoxCpmMetalAdapter {
    pub fn new(config: &VoxCpmConfig, device: &Device) -> Result<Self> {
        log::info!(
            "VoxCPM: Initializing Metal-Optimized adapter with full Metal GPU and F16 precision..."
        );
...
impl TtsEngine for VoxCpmMetalAdapter {
