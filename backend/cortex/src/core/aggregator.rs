...
impl NewsAggregator {
    ...
    fn build_prompt(
        ...
        format!(
            ...
            5. **简洁衔接**：段落之间的过渡必须简洁直接，直接用新闻中的主体信息衔接。禁止使用\"我们先从...说起\"、\"说到...\"、\"转向...\"、\"值得关注\"等模板化语句。\n\
            6. **事实性表达**：优先使用包含新闻主体和关系的具体表述，避免\"此举旨在...\"、\"显示...\"等总结性、概括性的抽象语言。\n\
            7. **风格**：专业、客观，但具有会话感。\n\
            8. **禁止幻觉**：不要编造未提供的信息。\n\
            9. **格式**：纯文本，不要使用Markdown。\n\
            10. **标题要求**：请在输出的第一行生成一个标准标题，格式严格为：`TITLE: 内容关键词`。\n\
