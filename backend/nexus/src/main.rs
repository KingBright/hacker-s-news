...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/api/history", get(routes::history::get_history).post(routes::history::mark_played).delete(routes::history::unmark_played))
        .route("/api/auth/login", post(routes::auth::login))
        .route("/api/admin/users", post(routes::auth::create_user).get(routes::auth::list_users))
